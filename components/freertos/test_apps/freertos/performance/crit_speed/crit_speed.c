...
static void Test_CriticalSectionSpeed(void)
{
    ...
    portSPINLOCK_TYPE task_spinlock = SPINLOCK_INITIALIZER;
    portSPINLOCK_TYPE isr_spinlock = SPINLOCK_INITIALIZER;
#endif
...
#if portUSING_GRANULAR_LOCKS == 1
        taskDATA_GROUP_ENTER_CRITICAL(&task_spinlock, &isr_spinlock);
#else
        taskENTER_CRITICAL();
#endif
...
#if CONFIG_FREERTOS_SMP
#if portUSING_GRANULAR_LOCKS == 1
        taskDATA_GROUP_EXIT_CRITICAL(&task_spinlock, &isr_spinlock);
#else
        taskEXIT_CRITICAL();
#endif
#else /* CONFIG_FREERTOS_SMP */
