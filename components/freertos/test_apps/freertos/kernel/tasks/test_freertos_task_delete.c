 ...
 *
 * Note: No assertions on p->deleted inside the loop. With SMP granular locks,
 * tasks may be deferred for deletion (e.g., when preemption is disabled) and
 * can briefly continue running before the deferred action takes effect. The
 * test validates correct cleanup via heap checks in the test case body.
 */
