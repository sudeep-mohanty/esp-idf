...
#if ( configNUMBER_OF_CORES > 1 )
#define portGET_TASK_LOCK( xCoreID )                         do { (void)xCoreID; vPortTakeLock(&port_xTaskLock); } while(0)
#define portRELEASE_TASK_LOCK( xCoreID )                     do { (void)xCoreID; vPortReleaseLock(&port_xTaskLock); } while(0)
#define portGET_ISR_LOCK( xCoreID )                          do { (void)xCoreID; vPortTakeLock(&port_xISRLock); } while(0)
#define portRELEASE_ISR_LOCK( xCoreID )                      do { (void)xCoreID); vPortReleaseLock(&port_xISRLock); } while(0)
#endif /* configNUMBER_OF_CORES > 1 */
