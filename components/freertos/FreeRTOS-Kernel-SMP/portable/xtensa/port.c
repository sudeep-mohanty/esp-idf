...
{
    vPortLockDataGroup(&port_xUserTaskLock, &port_xUserISRLock);
}

UBaseType_t uxPortEnterCriticalFromISRUser(void)
{
    return uxPortLockDataGroupFromISR(&port_xUserISRLock);
}

void vPortExitCriticalUser(void)
{
    vPortUnlockDataGroup(&port_xUserTaskLock, &port_xUserISRLock);
}

void vPortExitCriticalFromISRUser(UBaseType_t uxSavedInterruptStatus)
{
    vPortUnlockDataGroupFromISR(uxSavedInterruptStatus, &port_xUserISRLock);
}
